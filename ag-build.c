...
static void build_single(struct ag_project* project, int argc, const char** argv) {
    build_component(project, extract_component(project, argc, argv));
}
...
void build(int argc, const char** argv) {
    ...
    if (1 <= argc) {
        if (!strcmp("up", *argv)) {
            ...
            build_single(project, argc, argv);
        }
    } else {
